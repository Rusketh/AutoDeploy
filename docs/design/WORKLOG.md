...
## 2026-06-02 — End-to-end UEFI Secure Boot (signed kernel via shim)

**WHAT.** Completed the Secure Boot story started in the iPXE migration: the
chain is now verified firmware → bootloader → kernel. `boot.ipxe` (generated by
`server/internal/api/ipxe.go`) detects Secure Boot per machine via the
`${efi/SecureBoot}` EFI variable and, when on, registers a Microsoft-signed
Ubuntu shim (`autodeploy-shim.efi`) with iPXE's `shim` command so the
Canonical-signed kernel passes verification before booting. The boot-image
build (`release.yml` bootimage job + `build-bootimage.yml`) now installs
`shim-signed` and ships `/usr/lib/shim/shimx64.efi.signed*` as
`autodeploy-shim.efi`; `fetch-ipxe.sh` fetches it alongside the kernel/initrd.
Docs (`pxe-and-boot.md` Secure Boot section rewritten, `troubleshooting.md`,
portal hint, installer banner) updated to describe the verified chain and its
limits. `build-initramfs.sh` gained a lockdown/signed-module note.

**WHY (assumptions / decisions).**
- Path A (distro-signed kernel + distro shim, the netboot.xyz pattern) was
  chosen over self-signing+key-enrolment (per-machine MOK friction) or our own
  MS-signed shim (months-long shim-review). User approved: additive rollout,
  base kernel left to implementation.
- KEY REALISATION: AutoDeploy's boot image is *already* built on Ubuntu
  `linux-generic`, whose vmlinuz is Canonical-signed with signed modules. So no
  new kernel asset was needed — only the Ubuntu shim plus the `shim` command.
  Truly additive: the non-Secure-Boot path is byte-for-byte unchanged.
- `${efi/SecureBoot}` lets a single `boot.ipxe` auto-route per machine (SB-on,
  SB-off, BIOS), so no operator toggle. Detection uses `:int8` compared to 1;
  on BIOS/unset the iseq fails gracefully and the kernel boots directly.
- Only the kernel is signature-verified; the initramfs and cmdline are not.
  This buys Secure Boot *compatibility*, not tamper-proof pre-boot integrity —
  documented explicitly. Kernel lockdown means only Canonical-signed modules
  load under SB; out-of-tree drivers require SB off (documented).

**STATE.** `go build ./...`, `go vet`, `gofmt`, full `go test ./...` green
(boot.ipxe test extended to assert the SecureBoot branch + shim line). Both
workflow YAMLs parse. Shell scripts syntax-checked. NOT yet validated on real
Secure Boot hardware/OVMF — the `${efi/SecureBoot:int8}` detection and the
shim→kernel verification must be confirmed on an SB machine/VM before relying on
it; failure mode is graceful (drops to iPXE shell with a Secure-Boot hint).

**NEXT.** Validate on an OVMF Secure Boot VM with MS keys enrolled. Optional
